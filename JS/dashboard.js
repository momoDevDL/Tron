    ...
    $(document).ready(function () {
        ...
        $('.niveauElo p:first').html("Niveau : " + niveau);

        $('body').on("click", "#modifyProfile", function () {
            let niveau = $('.niveauElo p:first').html();
            let elo = $('.niveauElo p:last').html();
        ...
        $('body').on('click', '#1V1', function () {
            ...
            socket.on('connect', function () {
                if (niveau > 2) {
                    ...
                    $('#rechercheMatch p').html("Votre niveau ne vous permet pas de jouer en ligne veuillez augmenter votre niveau en s'entrainant avec L'IA avant de s'aventurer dans notre mode de jeu en ligne");
                    $('#rechercheMatch').fadeIn();
            ...
            */
            ...
            function nouvellePartie() {
                ...
                if (nbrManche <= 0 || (nbrManche == 1 && (score[0] >= 2 || score[1] >= 2))) {
                    ...
                    socket.emit('score', NBR_MANCHE - nbrManche, score[ID_joueur - 1], ID_joueur, indiceRoom, NB_partJoue, Elo, XP);
                } else {
                    BoutonReady();
                }
            }
            ...
            socket.on('Replay', () => {
                $.ajax({
                    ...
                    complete: function (data) {
                        console.log('var Replay est créér dans la session actuel : completed');
                        console.log(data);
                    },
            ...
            $('body').on('click', '#btnQuit', () => {
                if (ID_joueur == 1)
                    ...
                    score[0] += nbrManche;
                socket.emit('JQuit', NBR_MANCHE - nbrManche, score, ID_joueur, indiceRoom);
            });
